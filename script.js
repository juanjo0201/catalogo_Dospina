...
async function cargarProductos() {
  try {
    ...
    grid.innerHTML =
      '<p class="empty-state">No se pudo cargar el catálogo. Revisa que data/productos.json exista.</p>';
...
function inicializarFiltros() {
  ...
  categoryRow.innerHTML = categorias
    .map(
      (cat) => `
...
function actualizarFiltroTalla() {
  ...
  const tallas = [
    ...new Set(productosCategoria.map((p) => p.talla).filter(Boolean)),
  ].sort();
...
function mensajeWhatsapp(producto) {
  ...
  const texto = `Hola, estoy interesad@ en este producto:
...
function abrirModal(id) {
  ...
  const detalles = [];
  if (producto.talla)
    detalles.push(`<li><strong>Talla:</strong> ${producto.talla}</li>`);
  if (producto.material)
    detalles.push(`<li><strong>Material:</strong> ${producto.material}</li>`);
...
function revisarLinkDirecto() {
  ...
  const idBuscado = parseInt(
    window.location.hash.replace("#producto-", ""),
    10,
  );
  const indice = productosFiltrados.findIndex((p) => p.id === idBuscado);

  // Si el producto no existe en el filtro actual (por defecto es "Todo", así que
  // debería encontrarlo siempre que el producto exista en el catálogo)
  if (indice === -1) return;

  // Calcula en qué página de la paginación cae ese producto, y salta ahí
  paginaActual = Math.floor(indice / PRODUCTOS_POR_PAGINA) + 1;
  renderizarGrid();

  // Ahora sí, ya está dibujado en el DOM: lo busca y lo resalta
  setTimeout(() => {
...
function configurarBotonesFijos() {
  const mensajeGeneral = encodeURIComponent(
    "Hola, quiero más información sobre el catálogo de D'Ospina Alta Costura.",
  );
  document.getElementById("btn-whatsapp-flotante").href =
    `https://wa.me/${CONFIG.whatsapp}?text=${mensajeGeneral}`;
  document.getElementById("btn-whatsapp-footer").href =
    `https://wa.me/${CONFIG.whatsapp}?text=${mensajeGeneral}`;
  document.getElementById("btn-instagram-footer").href =
